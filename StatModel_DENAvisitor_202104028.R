...
modlCLMM[[8]]  = clmm2(RC_Acceptability  ~ ClipSeqence + Flight_interest                        , random = ID, data=dataIn3, nAGQ=10, na.action = na.omit, Hess =TRUE )
modlCLMM[[9]]  = clmm2(RC_Acceptability   ~ ClipSeqence + Location                              , random = ID, data=dataIn3, nAGQ=10, na.action = na.omit, Hess =TRUE )
modlCLMM[[10]] = clmm2(RC_Acceptability  ~ Flight_interest + Location                           , random = ID, data=dataIn3, nAGQ=10, na.action = na.omit, Hess =TRUE )
#three variables
modlCLMM[[11]] = clmm2(RC_Acceptability  ~ (LAeq30s) + ClipSeqence + Flight_interest            , random = ID, data=dataIn3, nAGQ=10, na.action = na.omit, Hess =TRUE )
modlCLMM[[12]] = clmm2(RC_Acceptability  ~ (LAeq30s) + ClipSeqence + Location                   , random = ID, data=dataIn3, nAGQ=10, na.action = na.omit, Hess =TRUE )
modlCLMM[[13]] = clmm2(RC_Acceptability  ~ (LAeq30s) + Flight_interest + Location               , random = ID, data=dataIn3, nAGQ=10, na.action = na.omit, Hess =TRUE )
modlCLMM[[14]] = clmm2(RC_Acceptability  ~ ClipSeqence + Flight_interest + Location             , random = ID, data=dataIn3, nAGQ=10, na.action = na.omit, Hess =TRUE )
...
#-------------------------------------------------------------------------------------------------------
# report out a table of AIC values for each model 
AIC_CLMM = NULL
...
fit1 = (cbind(dataIn2,fitted(fm2)) )
plot(fit1$ID,fit1$`fitted(fm2)`)
...
mat = expand.grid(ID = qnorm(0.95) * c(-1,0,1) * fm2$stDev, 
                  ...
                  Flight_interest  = c(0, 1*fm2$beta[6]) )
...
#save out and update/format in illustrator

#------------------------------------------------------------------------------------------------
#can I predict probability of 0|1 (acceptibility) for sound levels 20-80, average visitor, no flight interest
#------------------------------------------------------------------------------------------------
#(Average visitor, NO flight interest)
mat2 = expand.grid(ID = 0, #average visitor
                  ClipSeqence      = 1*fm2$beta[2], #clip sequence 2
                  LAeq30s          = seq(from= 20, to= 90,by=1) * fm2$beta[1], #possible sound levels 
                  Flight_interest  = 0 ) #NO fight interest, Yes flight interest = 1*fm2$beta[3]
pred.mat2 = as.data.frame (pred(eta=rowSums(mat2),theta=fm2$Theta))
colnames(pred.mat2) = rownames(as.data.frame( fm2$Theta) )
DBs = as.data.frame(seq(from= 20, to= 90,by=1))
colnames(DBs)="LAEQ"
...
#(Average visitor, YES flight interest)
mat2Y = expand.grid(ID = 0, #average visitor
                   ClipSeqence      = 1*fm2$beta[2], #clip sequence 2
                   LAeq30s          = seq(from= 20, to= 90,by=1) * fm2$beta[1], #possible sound levels 
                   Flight_interest  = 1*fm2$beta[6] ) #NO fight interest = 0, Yes flight interest = 1*fm2$beta[6]
pred.mat2Y = as.data.frame (pred(eta=rowSums(mat2Y),theta=fm2$Theta))
colnames(pred.mat2Y) = rownames(as.data.frame( fm2$Theta) )
DBs = as.data.frame(seq(from= 20, to= 90,by=1))
colnames(DBs)="LAEQ"
pred.mat3Y = cbind(pred.mat2Y,DBs)
ggplot(pred.mat3Y, aes(LAEQ,`-1|0`))+
  geom_line()+
  ggtitle("Probability of rating as unacceptible (0 or less) (Average visitor, YES flight interest)")
pred.mat2mY = reshape::melt(pred.mat2Y)
DBsm = rbind(DBs,DBs,DBs,DBs,DBs,DBs,DBs,DBs)
pred.mat2mY = cbind(pred.mat2mY,DBsm)
ggplot(pred.mat2mY, aes(LAEQ,value,color=variable))+
  geom_line()+
  ylab("Probability of rating")+
  theme_minimal()

#------------------------------------------------------------------------------------------------
#FOR DAVYD-map: sound level with probability (look up table to convert values) 
#------------------------------------------------------------------------------------------------
#(Average visitor, NO flight interest)
pred.mat2mA = pred.mat2m[ pred.mat2m$variable ==  '0|1',]
ggplot(pred.mat2mA, aes(LAEQ,value))+
  geom_line()+
  ylab("Probability of rating")+
  theme_minimal()
#Table of max probabilities for each category
PreRate = unique( pred.mat2m$variable )
ProbOut = NA
for (ii in 1:length(PreRate)){
  tmp = pred.mat2m[ pred.mat2m$variable ==  PreRate[ii],] 
  idx = which.max( (tmp$value) )      
  ProbOut = rbind(ProbOut, c(as.character(tmp$variable[idx]),tmp$value[idx],tmp$LAEQ[idx]) )
}
ProbOut
#So,can I use the ranges to say rating for each category... yes?
ProbNo = rbind ( pred.mat2m[ pred.mat2m$variable ==  as.character(PreRate[1]) & pred.mat2m$LAEQ >66 ,],
                 pred.mat2m[ pred.mat2m$variable ==  as.character(PreRate[2]) & pred.mat2m$LAEQ <=66 & pred.mat2m$LAEQ >58,],
                 pred.mat2m[ pred.mat2m$variable ==  as.character(PreRate[3]) & pred.mat2m$LAEQ <=58 & pred.mat2m$LAEQ >51,],
                 pred.mat2m[ pred.mat2m$variable ==  as.character(PreRate[4]) & pred.mat2m$LAEQ <=51 & pred.mat2m$LAEQ >45,],
                 pred.mat2m[ pred.mat2m$variable ==  as.character(PreRate[5]) & pred.mat2m$LAEQ <=45 & pred.mat2m$LAEQ >38,],
                 pred.mat2m[ pred.mat2m$variable ==  as.character(PreRate[6]) & pred.mat2m$LAEQ <=38 & pred.mat2m$LAEQ >27,],
                 pred.mat2m[ pred.mat2m$variable ==  as.character(PreRate[7]) & pred.mat2m$LAEQ <=27 & pred.mat2m$LAEQ >20,],
                 pred.mat2m[ pred.mat2m$variable ==  as.character(PreRate[8]) & pred.mat2m$LAEQ <=20,])
ProbNo = ProbNo %>% arrange(desc(LAEQ))

#(Average visitor, YES flight interest)
pred.mat2mAYA = pred.mat2mY[ pred.mat2mY$variable ==  '0|1',]
ggplot(pred.mat2mAYA, aes(LAEQ,value))+
  geom_line()+
  ylab("Probability of rating")+
  theme_minimal()
#Table of max probabilities for each category
PreRate = unique( pred.mat2mY$variable )
ProbOutY = NA
for (ii in 1:length(PreRate)){
  tmp = pred.mat2mY[ pred.mat2mY$variable ==  PreRate[ii],] 
  idx = which.max( (tmp$value) )      
  ProbOutY = rbind(ProbOutY, c(as.character(tmp$variable[idx]),tmp$value[idx],tmp$LAEQ[idx]) )
}
ProbOutY
#So,can I use the ranges to say rating for each category... yes?
ProbY= rbind (   pred.mat2mY[ pred.mat2mY$variable ==  as.character(PreRate[1]) & pred.mat2mY$LAEQ >72 ,],
                 pred.mat2mY[ pred.mat2mY$variable ==  as.character(PreRate[2]) & pred.mat2mY$LAEQ <=72 & pred.mat2mY$LAEQ >64,],
                 pred.mat2mY[ pred.mat2mY$variable ==  as.character(PreRate[3]) & pred.mat2mY$LAEQ <=64 & pred.mat2mY$LAEQ >57,],
                 pred.mat2mY[ pred.mat2mY$variable ==  as.character(PreRate[4]) & pred.mat2mY$LAEQ <=57 & pred.mat2mY$LAEQ >51,],
                 pred.mat2mY[ pred.mat2mY$variable ==  as.character(PreRate[5]) & pred.mat2mY$LAEQ <=51 & pred.mat2mY$LAEQ >44,],
                 pred.mat2mY[ pred.mat2mY$variable ==  as.character(PreRate[6]) & pred.mat2mY$LAEQ <=44 & pred.mat2mY$LAEQ >33,],
                 pred.mat2mY[ pred.mat2mY$variable ==  as.character(PreRate[7]) & pred.mat2mY$LAEQ <=33 & pred.mat2mY$LAEQ >20,],
                 pred.mat2mY[ pred.mat2mY$variable ==  as.character(PreRate[8]) & pred.mat2mY$LAEQ <=20,])
ProbY = ProbY %>% arrange(desc(LAEQ))
colnames(ProbY) = c("RatingNo", "ProbYes","SPLYes")

ProbOut  = cbind(ProbNo, ProbY)
colnames(ProbOut) = c("RatingNo", "ProbNo","SPLNo","RatingYes","ProbYes","SPLYes")
write.csv(ProbOut,"E:\\RESEARCH\\NSNSD_Projects\\LF_DENAanalysis\\DENAvisitor_Output_ProbRatingSPL.csv")
