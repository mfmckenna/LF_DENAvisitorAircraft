#UPDATE!!! This code was updated by Lauren Abbott (Ferguson) 
# on 6/10/2018--The team aims to test the model and try to account for the non-randomness in clip order
...
par(mfcol=c(2,2), mar=c(2,2,2,0.5), oma=c(0.5,0.5,0.5,0.5))
#all clips model
...
gam3 <-	gam(RC_Acceptability ~   s(LAeq30s, ClipSeqence, bs = "fs"), data = ALL2, method = "REML") #"fs"=Factor Smooth for the categorical/continuous interaction term
...
# BUILDING a more complete model dataset to evaluate 
# Cumulative link mixed models- CLMM (based on Kurt's GRCA model)
library(ordinal)
# GAM models
# GLMM models 
...
hmods00 = lapply(modls, function(mm) glmer(mm, data=ALL2, family="gaussian", nAGQ=10) )
mdsum00 <- summary(model.avg(hmods00))
capture.output(mdsum00,file=paste(srcdir,"AllTrailHeli1.txt",sep=""))
hmods0 <- lapply(modls0b,function(mm) glmer(mm,data=grall[ixna], family="binomial", nAGQ=10)) #anonymous
...
# # one variable
# RC_Acceptability  ~ LAeq30s,
# RC_Acceptability  ~ ClipSeqence,
# RC_Acceptability  ~ Flight_interest,
# RC_Acceptability  ~ Location,
# # two variables
# RC_Acceptability  ~ LAeq30s + ClipSeqence,
# RC_Acceptability  ~ LAeq30s + Flight_interest,
# RC_Acceptability  ~ LAeq30s + Location,
# RC_Acceptability  ~ ClipSeqence + Flight_interest,
# RC_Acceptability  ~ ClipSeqence + Location,
# RC_Acceptability  ~ Flight_interest + Location,
# #three variables
# RC_Acceptability  ~ LAeq30s + ClipSeqence + Flight_interest,
# RC_Acceptability  ~ LAeq30s + ClipSeqence + Location,
# RC_Acceptability  ~ LAeq30s + Flight_interest + Location,
# RC_Acceptability  ~ ClipSeqence + Flight_interest + Location,
# # four variables
# RC_Acceptability  ~ LAeq30s + ClipSeqence + Flight_interest + Location
